...
def parallel_transport_along_edges(x, edge_index, W_connection):
    ...
    # Получаем features исходных вершин
    ...
    if W_connection.dim() == 2:
        ...
        raise ValueError(f"Неверная форма W_connection: {W_connection.shape}")

    # Агрегируем по целевым вершинам (БЕЗ torch_scatter)
...
class ParallelTransportLayer(nn.Module):
    ...
    def forward(self, x, edge_index, W_connection, edge_weight=None):
        ...
        if self.normalize:
            # Вычисляем степени вершин (БЕЗ torch_scatter)
            src, dst = edge_index
